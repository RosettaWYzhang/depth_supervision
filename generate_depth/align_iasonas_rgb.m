function [depth_norm, confidence] = align_iasonas_rgb(depthim, rgbim, point2d_incolor)
...
composite1 = chk.*depth_norm + (1.-chk).*double(confidence);
composite2 = chk.*depth_norm + (1.-chk).*double(rgb2gray(rgbim))/256;
composite3 = chk.*confidence + (1.-chk).*double(rgb2gray(rgbim))/256;
end
